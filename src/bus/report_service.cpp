...
expected<bool, string> ReportService::createReport(
    ...
    doc << "title" << title
        ...
        << "region" << region
        << "resolved" << false
        << "from" << userView["_id"].get_oid().value
        << "created_at" << now;

    if (!ReportRepository::insertReport(doc.view())) {
        return unexpected("Failed to create report");
    }
